...
|---|---|---|---|---|
| Portal clientes/proyectos (`src/pages/portal/*`, `src/services/api.ts`) | Mixto: API para auth/listados/overview (`api/*`) + mezcla local en landing (se combinan proyectos de `ProjectContext` con públicos API). | `POST /api/auth/clients-area`, `POST /api/auth/project/:slug`, `POST /api/auth/operator`, `GET /api/clients`, `GET /api/public/clients`, `GET /api/:slug/overview`, `POST /api/tools/run/:tool`. | **Medio**: ya existe contrato API, pero hay lógica de unión local+remoto en frontend que puede generar divergencias de estado. | **P0** |
