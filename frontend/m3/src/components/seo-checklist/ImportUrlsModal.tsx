...
export const ImportUrlsModal: React.FC<Props> = ({ isOpen, onClose, onImport, existingPages }) => {
  ...
  const handleImport = () => {
    ...
    lines.forEach(line => {
      ...
      if (parts.length >= 2) { // Minimum URL and KW
    ...
    if (newPages.length > 0) {
        ...
        if (inputText.trim().length > 0) {
             alert('No se importaron URLs nuevas. Es posible que todas sean duplicadas o el formato sea incorrecto.');
        } else {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 flex-1 overflow-y-auto">
            ...
            <div className="mt-4 flex items-center gap-2">
                ...
                />
                <label htmlFor="ignoreDuplicates" className="text-sm text-slate-700 dark:text-slate-300 cursor-pointer select-none">
